---
title: 为什么我们需要UniTask？
tags:
  - C#
  - UniTask
  - Unity
  - 异步
createTime: '2022-04-27 17:02:00'
permalink: /article/tech/why-we-need-unitask/
---
我之前曾经提到过一个Unity插件叫做UniTask，是用来提供Unity异步操作用的。但是我想各位可能不太明确的一个问题是，为什么Unity需要使用到异步操作？或者还有人会问我，什么是异步？为了讲清楚这个东西，我将会从C#的异步和Unity多线程上的陷阱开始，为大家一步一步梳理UniTask应用场景。 多线程与异步
...
UniTask是目前解决这个问题的最佳方案，它在保证Unity线程安全的前提下还支持C#的异步编程模型，而且不需要依赖MonoBehaviour便可以运行。 不过我想偷个小懒，这个是[用法的文档](https://github.com/Cysharp/UniTask#readme)。
...
public async Task Transit()
{
    ...
    try
    {
        awaitgroup.DOFade(1, duration.x).ToUniTask(); // 这里是Dotween的动画API，还支持UniTask
    ...
    awaitgroup.DOFade(0, duration.y).ToUniTask(); // 然后淡出动画
...
- 通过对UniTask.Yield()进行await，可以获取到远比MonoBehaviour更加细致的生命周期（见 [Github项目主页](https://github.com/Cysharp/UniTask#playerloop) ）。
